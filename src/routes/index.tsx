  ...
  const processText = async (text: string) => {
    ...
    try {
      const storyList = stories
        .map((s) => `- ${s.title} (المستوى: ${s.level === "beginner" ? "مبتدئ" : s.level === "intermediate" ? "متوسط" : "متقدم"}, النوع: ${s.genre})`)
        .join("\n");
      ...
      const response = await fetch("https://openrouter.ai/api/v1/chat/completions", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "system",
              content: `أنت مساعد تعلم اللغة الإنجليزية للناطقين بالعربية في تطبيق Booklish.
القصص المتاحة:
${storyList}

عندما يطلب المستخدم اقتراح قصة، اقترح 1-3 قصص من القائمة أعلاه بناءً على مستواه أو اهتمامه.
إذا سأل عن قاعدة نحوية أو كلمة إنجليزية، اشرح له بالعربية ببساطة.
أجب دائماً بالعربية في 2-3 جمل بأسلوب ودي ومشجع.
إذا اقترحت قصصاً، أضف في نهاية ردك: [SUGGEST: العنوان1, العنوان2]
استخدم العناوين الإنجليزية الدقيقة كما في القائمة.`,
            },
      ...
      const msg = ar
        ? "حدث خطأ في الاتصال. تأكد من إضافة VITE_OPENROUTER_API_KEY في إعدادات Vercel."
        : "Connection error. Check VITE_OPENROUTER_API_KEY in Vercel settings.";
      setReply(msg);
    } finally {
      setThinking(false);
    }
  ...
  const startListening = () => {
    ...
    // دائماً استمع بالعربية لأن المستخدمين عرب
    ...
    recognition.onerror = (e: any) => {
      // إذا فشل ar-SA جرّب en-US (للمستخدمين الذين يتحدثون إنجليزياً)
  ...
  return (
    <section className="mb-8 overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
      ...
      <div className="px-5 py-4 border-b border-border flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-muted-foreground truncate">
            {ar ? "تحدث بالعربية واقترح لك قصة مناسبة" : "Speak to get story suggestions"}
          </p>
        </div>
      </div>
      ...
      <div className="p-5 space-y-4">
          ...
          >
            ...
            <span className="relative z-10">
              {thinking
                ...
                : listening
                ? ar ? "يستمع... (3 ث صمت للإرسال)" : "Listening... (3s silence)"
        ...
        {/* ما سمعه */}
        {transcript && (
          <div className="flex gap-2 items-start" dir={ar ? "rtl" : "ltr"}>
    ...
    <main className="mx-auto max-w-5xl px-4 pb-24 pt-8 sm:pt-14">
      ...
      {continueStory && (
        <section className="mb-8">
          ...
          >
            <div
              className={`grid h-14 w-14 shrink-0 place-items-center rounded-xl bg-gradient-to-br ${continueStory.coverHue} text-2xl shadow-sm`}
            >
              {continueStory.cover}
            </div>
            <div className="min-w-0 flex-1">
              ...
              <div className="h-1.5 w-full overflow-hidden rounded-full bg-muted">
                <div
                  ...
                  style={{ width: `${continueEntry![1].pct}%` }}
                />
              </div>
      ...
      {/* Voice Assistant — أسفل الصفحة بعد المحتوى الرئيسي */}
        ...
        <div className="border-b border-border px-4 py-3">
          <div className="flex items-center justify-between text-sm">
            <span className="font-medium">
              {level.icon} {ar ? level.nameAr : level.nameEn}
            </span>
            {xpToNext > 0 && (
        ...
        <section className="mb-8 overflow-hidden rounded-2xl border border-primary/30 shadow-sm">
          <div className="bg-gradient-to-br from-primary/15 via-primary/8 to-transparent p-5">
            ...
            <ul className="mb-4 space-y-1 text-sm text-muted-foreground">
              <li>✓ {ar ? "جميع القصص والمستويات" : "All stories & levels"}</li>
